...
import {
  PASTOR_PERSONA, PASTOR_SETUP, SIGNED_OUT_SETUP,
  profileFromStaff, registerCloudIdentity, sameProfile, signOutStaff,
  type StaffRecord,
} from './staffIdentity';
...
export function useStaffIdentity() {
  ...
  /**
   * Stamp the pastor persona + roster name/email.
   ...
   * form and its warning) and never overrides a persona the user chose.
   */
  const applyStaffIdentity = useCallback(async (staff: StaffRecord, opts?: { boot?: boolean }) => {
    ...
    if (!boot && currentEmail !== staffEmail) {
      ...
      clearSessionToken();
      resetSyncSession();
      await registerCloudIdentity(profileFromStaff(staff, latest.current.userProfile), PASTOR_PERSONA);
    }
    ...
    const next = profileFromStaff(staff, current);
  ...
  /** Sign out: revoke + forget the staff token, back to the Church Member persona.
   *  The profile (and its cloud backup) stays — that is the person, not the role. */
